...
public class Odometer extends Thread {
  ...
  public void run() {
    ...
    while (true) {
      ...
      distL = Math.PI * OdometryLab.WHEEL_RADIUS * (nowTachoL - lastTachoL) / 180; // Displacement
      distR = Math.PI * OdometryLab.WHEEL_RADIUS * (nowTachoR - lastTachoR) / 180;

      lastTachoL = nowTachoL; // Update previous Tachometer readings.
      lastTachoR = nowTachoR;

      deltaD = 0.5 * (distL + distR); // Average displacement
      deltaT = (distL - distR) / OdometryLab.TRACK;// Change in Theta, 14.7 is wheelbase
